...
export async function extractReceiptData(imageFile, countryCode = 'SE') {
  try {
    console.log('=== Receipt Extraction Debug ===');
    console.log('API Key exists:', !!import.meta.env.VITE_ANTHROPIC_API_KEY);
    console.log('API Key first 20 chars:', import.meta.env.VITE_ANTHROPIC_API_KEY?.substring(0, 20));
    console.log('API Key length:', import.meta.env.VITE_ANTHROPIC_API_KEY?.length);
    console.log('Country code:', countryCode);
    console.log('File type:', imageFile.type);
    console.log('File size:', imageFile.size);

    if (imageFile.type === 'application/pdf') {
      throw new Error('PDF files are not supported for AI extraction. Please upload an image file (JPG or PNG) or use manual entry.');
    }

    const apiKey = import.meta.env.VITE_ANTHROPIC_API_KEY;

    if (!apiKey || apiKey === 'your_api_key_here') {
      throw new Error('Anthropic API key not configured. Please add VITE_ANTHROPIC_API_KEY to your .env file. Get your key from https://console.anthropic.com/');
    }

    const base64Image = await convertImageToBase64(imageFile);

    let imageType = imageFile.type || 'image/jpeg';
    ...
    console.log('Calling Claude API for receipt extraction...');
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error('Invalid API key. Please check your VITE_ANTHROPIC_API_KEY in the .env file.');
    ...
    try {
      ...
      throw new Error('Failed to parse extracted data. The AI response was not in the expected format.');
    }
    ...
    console.warn('API call failed, using smart mock extraction:', error.message);
    ...
    return {
      merchantName: "Mock Merchant (API blocked)",
      date: new Date().toISOString().split('T')[0],
      totalAmount: 100.00,
      vatAmount: 20.00,
      vatRate: 25,
      currency: countryCode === 'US' ? 'USD' :
                countryCode === 'UK' ? 'GBP' :
      ...
      countryCode: countryCode,
      confidence: 50,
      note: 'Using mock data - API call blocked by CORS'
    };
  }
}
